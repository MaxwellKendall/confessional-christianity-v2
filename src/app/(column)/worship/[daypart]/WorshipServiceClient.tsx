...
// The service itself (mockups 11b–11i): eight steps, one per screen, each a
// quiet single-purpose surface rendered from the liturgy's typed elements —
// this component is the one renderer all eight share. Step 5 (Professing
// Faith) is the deliberate exception: a hand-off card into the untouched
// session screen (8a), which returns here at step 6 when the question is
// answered. Element sources render as a footnote-register line, never a card
// competing with the text being prayed or sung.
...
import {
  getService, type Daypart, type ElementSource, type WorshipElement,
} from '@/lib/worship';
...
function ElementView({
  element, daypart, handoff,
}: { element: WorshipElement; daypart: Daypart; handoff: Handoff | null }) {
  switch (element.type) {
    case 'scripture':
      return (
        <div>
    ...
    case 'catechism': {
      ...
      return (
        <div className="w-full">
          {element.lede && <Lede text={element.lede} />}
          <Link
            href={`/programs/${handoff.program.slug}/session?worship=${daypart}`}
...
export function WorshipServiceClient({ daypart }: { daypart: Daypart }) {
  ...
  // Resolved once per visit so a service never changes mid-worship, even
  // across midnight.
  ...
  const stepNumber = Number.isInteger(requested)
    ...
    : 1;
  const step = service.steps[stepNumber - 1];
  ...
  return (
    <div className="flex min-h-[calc(100dvh-7rem)] flex-col">
      <div className="flex items-center justify-between px-6 pt-5">
        {stepNumber === 1 ? (
          <Link href="/worship" aria-label="Exit worship" className="font-display text-[15px] text-ink no-underline">
            ✕
          </Link>
        ) : (
          <Link
            href={`/worship/${daypart}?step=${stepNumber - 1}`}
            aria-label="Back"
      ...
      <div className={`flex flex-1 flex-col items-center px-8 py-6 text-center ${isPromptsStep ? '' : 'justify-center'}`}>
        ...
        <div className="flex w-full flex-col gap-7">
          {step.elements.map((element, i) => (
            <ElementView key={i} element={element} daypart={daypart} handoff={handoff} />
          ))}
        </div>
      </div>
      ...
      <div className="border-t border-hairline px-6 pt-4 pb-9 text-center">
        {isCatechismStep ? (
          <span className="font-body text-[11.5px] italic text-muted">
            {`The question returns to Family Worship at step ${stepNumber + 1} when you’ve finished`}
          </span>
          ...
          <Link href={`/worship/${daypart}/complete`} className="action-button">
            Finish Worship
          </Link>
        )}
      </div>
